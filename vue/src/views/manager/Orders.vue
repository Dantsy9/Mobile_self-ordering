      ...
      <el-table :data="tableData" strip @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="status" label="订单状态">
          <template v-slot="scope">
            ...
            <el-tag type="primary" v-if="scope.row.status === '待出餐'">待出餐</el-tag>
        ...
        <el-table-column prop="tableNum" label="下单桌号"></el-table-column>
        ...
        <el-table-column label="操作" align="center" width="160" fixed="right">
          <template v-slot="scope">
            <el-button size="mini" type="info" @click="serveMeals(scope.row, '已完成')" v-if="scope.row.status === '待出餐' && user.role === 'BUSINESS'">出餐</el-button>
            <el-button size="mini" type="danger" @click="refund(scope.row, '已退款')" v-if="scope.row.status === '已完成'">退款</el-button>
...
<!--            <el-option v-for="item in ['已取消', '待支付', '待出餐', '已出餐', '已完成','待评价','已退款']" :key="item" :value="item"></el-option>-->
<!--          </el-select>-->
<!--        </el-form-item>-->
...
<!--          </el-select>-->
<!--        </el-form-item>-->
<!--        <el-form-item label="下单桌号" prop="tableNum">-->
<!--          <el-input v-model="form.tableNum" placeholder="下单桌号" disabled></el-input>-->
<!--        </el-form-item>-->
...
export default {
  ...
  methods: {
    // 出餐
    serveMeals(row) {
      let form = JSON.parse(JSON.stringify(row)) //防止数据更新失败，表格出错
      form.status = '已完成'
